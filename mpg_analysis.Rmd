...
First, let's get some intuitive information from a boxplot between mpg and am. Figure 1 is the plot(in Appendix). And we can have an intuition that **manual transimission is better for miles per gallon**. Next, we will search the right model for mpg.
...
### Summary
Manual transimission is better for mpg than automatic transimission. For people who want to save fuel using, manual transimission cars are preferred. And for car manufacturers, it's a good choice to optimize automatic transimission cars fuel use, to optimize the inner engine.
...
g <- g + geom_boxplot() + labs(x = "Transimission")
